...
class FingerspotAPIService:
    ...
    def get_attendance_data(self, device_config: Dict, start_date: datetime = None, end_date: datetime = None) -> List[FingerspotAttendance]:
        ...
        Args:
            ...
            start_date: Start date for data retrieval (default: today 00:00:00)
            end_date: End date for data retrieval (default: today 23:59:59)
        ...
        if start_date is None and end_date is None:
            ...
            logger.info(f"Using default date range for device {device_name}: {start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')}")
        ...
        try:
            ...
            for record in records:
                try:
                    ...
                    # Get punch/status code - handle 0 values properly
                    punch = None
                    if 'status_scan' in record:
                        punch = record['status_scan']
                    elif 'status' in record:
                        punch = record['status']
                    elif 'punch' in record:
                        punch = record['punch']
                    elif 'verify' in record:
                        punch = record['verify']
    ...
    def sync_device_data(self, device_config: Dict, start_date: datetime = None, end_date: datetime = None) -> Tuple[bool, str, List[Dict]]:
        ...
        Args:
            device_config: Device configuration
            start_date: Start date for sync (default: today 00:00:00)
            end_date: End date for sync (default: today 23:59:59)
        ...
        if start_date is None and end_date is None:
            ...
            start_date = today.replace(hour=0, minute=0, second=0, microsecond=0)
            end_date = today.replace(hour=23, minute=59, second=59, microsecond=999999)
            logger.info(f"Using default date range for sync {device_name}: {start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')}")
        
        try:
            # Get attendance data from API (will use our defaults if dates are None)
